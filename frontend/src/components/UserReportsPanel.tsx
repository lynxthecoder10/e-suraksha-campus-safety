...
import { getStatusKey, getStatusLabel } from '../lib/backendTypes';
  ...
  return (
    <>
      <div className="space-y-4">
        {reports.map((report) => (
          <Card key={report.reportId.toString()} className="hover:shadow-md transition-shadow">
            <CardContent className="p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-2">
                    <FileText className="h-4 w-4 text-primary" />
                    <span className="font-mono text-sm">Report #{report.reportId.toString()}</span>
                      ...
                      {statusHistory.map((entry, index) => (
                        <div key={index} className="flex items-start gap-3 text-sm p-2 bg-muted/50 rounded">
                          ...
                          <div className="flex-1">
                            <p className="font-medium capitalize">{getStatusLabel(entry.status)}</p>
